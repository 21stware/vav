/**
 * In-process vav-cli session loops. Kept off `vavcli.ts` so Playwright can
...
/** Stream a prompt the same way `vavcli -p` / the TTY REPL do. */
...
/** JSON-line RPC used by `vavcli --mode rpc`. */
export async function runVavcliRpc(
  phone: PhoneClient,
  session: string,
...
export async function runVavcliLines(
  phone: PhoneClient,
  session: string,
  lines: AsyncIterable<string> | Iterable<string>,
