 ...
 *
 * Desktop: a second window with its own store copy, bootstrapped light.
 * Chrome / web: the same Settings tree as an overlay on the live session
 * store — do not light-bootstrap again. Fields save on change; close via
 * the window chrome, overlay Close, or Escape (no Done footer).
 */
export default function SettingsWindow(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    // clear the live session's activeId and break Files preview.
    if (!useSessionStore.getState().ready) {
