...
import { isGitSnapshot, type GitChangeEntry, type GitSnapshot } from '@shared/git'
...
export function GitChangesPanel({
  ...
  useEffect(() => {
    ...
    onChrome({
      meta: `${snap.branch || t('git.detached', { head: snap.headShort ?? '?' })} · ${t('git.changeCount', { n: snap.changes?.length ?? 0 })}`,
      loading,
