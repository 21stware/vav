...
export function RemoteFolderPicker(): React.JSX.Element | null {
  ...
  const close = (): void => {
    if (pick?.purpose === 'directory') {
      window.dispatchEvent(new CustomEvent('vav:phone-folder-picked', { detail: { path: null } }))
    }
    closePicker()
  }
  ...
  const select = async (): Promise<void> => {
    ...
    if (pick.purpose === 'directory') {
      window.dispatchEvent(new CustomEvent('vav:phone-folder-picked', { detail: { path: chosen } }))
      closePicker()
      return
    }
    close()
