...
import {
  ...
  e2ePeekPopupMenu,
  e2ePopupMenu
} from '@main/e2ePopupMenu'
import { registerHapticsIpc } from '@main/haptics'
import {
  IPC,
  type Bootstrap,
...
import { createSwarmFinishAlert } from '@main/sound/swarmFinishAlert'
import { RemoteControlService } from '@main/remote/RemoteControlService'
import { DaemonAttachService } from '@main/daemon/DaemonAttachService'
...
import { ConversationStore } from '@main/store/ConversationStore'
import { LogStore } from '@main/store/LogStore'
import { createAppLogger, setAppLogger, appLog } from '@main/log/appLogger'
import { logTurnEvent } from '@main/log/turnLog'
import { attachDaemonLogStream, createDaemonLogRemote, registerLogIpc } from '@main/ipc/registerLogIpc'
...
import {
  ...
  syncOAuthProfiles
} from '@main/accounts/service'
import { accessTokenFromSnapshot } from '@main/accounts/credentials/parseKeychainSnapshot'
import {
  displayAccountLabel,
  agentIdOf,
  isVavProfile,
...
import { VavPackService } from '@main/store/VavPackService'
import { FileSessionStore } from '@main/store/FileSessionStore'
import { SwarmHistoryStore } from '@main/store/SwarmHistoryStore'
import { FileService } from '@main/fs/FileService'
import { installTrustedIpcGuard } from '@main/ipc/ipcTrust'
import { registerVcsIpc } from '@main/ipc/registerVcsIpc'
import { registerPluginIpc } from '@main/ipc/registerPluginIpc'
import { registerPtyIoIpc } from '@main/ipc/registerPtyIoIpc'
import { registerDialogIpc } from '@main/ipc/registerDialogIpc'
import { registerChangeSetIpc } from '@main/ipc/registerChangeSetIpc'
import { registerFilesIpc } from '@main/ipc/registerFilesIpc'
import { registerHostsIpc } from '@main/ipc/registerHostsIpc'
import { registerWindowIpc } from '@main/ipc/registerWindowIpc'
import { registerAgentIpc } from '@main/ipc/registerAgentIpc'
...
import { registerPreviewShellIpc } from '@main/ipc/registerPreviewShellIpc'
import { registerRuntimeIpc } from '@main/ipc/registerRuntimeIpc'
import { registerPtyCreateIpc } from '@main/ipc/registerPtyCreateIpc'
import { hostDisplayName as hostDisplayNameOf } from '@main/window/hostDisplay'
import { providerAccountViewOf } from '@main/window/providerAccountView'
import { findSwarmHistoryItem as findItemInSwarmHistory } from '@main/window/swarmHistoryFind'
import { machineIdFromRendererUrl } from '@main/window/machineFromUrl'
import { collectPreferredModelHosts, contextWindowForModelId, conversationModelHealPatch } from '@main/agent/modelContext'
import { activeTurnStatusFromPhase, awaitingNotifyKind, awaitingNotifyTitle, turnCompleteNotifyAction } from '@main/agent/agentEventNotify'
import { dialogConfirmOptions, revealSecretBoxOptions } from '@main/ipc/dialogOptions'
import { showParentedMessageBox, windowFromSender } from '@main/ipc/nativeDialog'
import { conversationIdForWorkdirs } from '@main/fs/conversationPath'
import { decorateHosts as decorateRemoteHosts } from '@main/host/decorateHosts'
import { conversationToMeta } from '@main/store/conversationMeta'
import {
...
import { wireExternalLinks as wireShellExternalLinks } from '@main/window/externalLinks'
import {
  hostWindowTitle as formatHostWindowTitle,
  mainWindowSize,
  rendererPrefs as buildRendererPrefs
} from '@main/window/rendererPrefs'
import {
...
import { isDisposableEphemeralSession, liveDetachedConversationIds } from '@main/window/ephemeralSession'
import {
...
import { trayDirLabel as formatTrayDirLabel, trayAgentLabel as formatTrayAgentLabel, pickAgentSessionTitle } from '@main/tray/trayLabels'
import { buildTrayPane } from '@main/tray/trayPane'
import {
  applyUnseenResultToMap,
  deleteUnseenForConversation,
  persistTrayResultUnseen,
  shouldHydratePersistedUnseen
} from '@main/tray/trayUnseen'
import { HostRegistry } from '@main/host'
import {
  conversationUsesLocalNode,
  localShellHostOf,
  waitForMountedLocalShell,
  workspaceHostForConversation
} from '@main/host/localShellHost'
import { copyAsFileSpawn, getInfoSpawn, openSpawn, previewSpawn, revealSpawn } from '@main/host/hostShell'
import { clipRoot, writeClipBytes } from '@main/fs/clipStore'
import { writePngToClipboard } from '@main/clipboardImage'
import { mapRemoteSessions, fallbackRemoteSession, buildRemoteThreadEvent } from '@main/remote/sessionList'
import { fanRemoteTurn as dispatchRemoteTurn } from '@main/remote/fanTurn'
import { listRemoteChildEntries, listRemoteRootEntries } from '@main/remote/dirBrowse'
import {
...
} from '@main/remote/sessionGate'
import { RemoteSendQueue } from '@main/remote/sendQueue'
import {
...
import { WorkingCopyService } from '@main/fs/WorkingCopyService'
import { FileAssociationService, formatIdForPath } from '@main/fs/FileAssociationService'
import { DocumentRetrievalService } from '@main/retrieval/DocumentRetrievalService'
import { DuckDbService } from '@main/fs/DuckDbService'
import { WebSearchService } from '@main/web/WebSearchService'
import { WebFetchService } from '@main/web/WebFetchService'
import { importSurfacePattern, surfacePatternFilePath } from '@main/importSurfacePattern'
import { ChangeSetStore } from '@main/agent/ChangeSetStore'
...
import { hostSessionHasConversation } from '@main/agent/hostSessionStore'
import { buildSwarmHistoryView } from '@main/agent/swarmHistoryView'
import {
...
} from '@main/agent/listHostModels'
import { SkillService } from '@main/agent/SkillService'
import { PluginService } from '@main/plugins/PluginService'
import { pluginAccessPaths } from '@main/plugins/pluginPaths'
import {
  cancelAgentInstall,
  clearAgentInstall,
...
} from '@main/agent/AgentInstaller'
import { clearHostAuthIdentityCache, readHostAccountInfo } from '@main/agent/hostAuth'
import { candidatesForHost } from '@main/agent/drivers'
import {
...
import { ensureMacOpenDirectoryService } from '@main/macOpenDirectoryService'
import { NotificationCenter } from '@main/notifications'
import { QuotaService } from '@main/quota/QuotaService'
import { fetchClaudeAccountQuota } from '@main/quota/claudeUsage'
...
import { fetchOpencodeAccountQuota } from '@main/quota/opencodeUsage'
...
} from '@main/analysis/analysisSnapshotCache'
...
const fileService = new FileService(
  ...
  (conversationId) =>
    workspaceHostForConversation(hostRegistry, conversationStore.get(conversationId)?.machineId).fs
)
...
const ptyManager = new PtyManager(
  ...
  (conversationId) =>
    workspaceHostForConversation(hostRegistry, conversationStore.get(conversationId)?.machineId).pty,
  (conversationId) =>
    conversationUsesLocalNode(hostRegistry, conversationStore.get(conversationId)?.machineId),
  (conversationId, candidates) => {
    const machineId = conversationStore.get(conversationId)?.machineId
    if (conversationUsesLocalNode(hostRegistry, machineId)) return null
    const hostId = isLocalMachine(machineId)
      ? localShellHostOf(hostRegistry)?.id
      : normalizeMachineId(machineId)
    if (!hostId) return null
    return daemonAttach.whichCached(hostId, candidates)
  }
)
liveAgentPanes.list = () =>
  ptyManager
...
function handleAgentEvent(event: TurnEvent): void {
  ...
  if (event.type === 'user') {
    ...
    syncScheduledJobFromConversation(event.conversationId, {
      prompt: event.message.content.trim() || undefined
    })
...
function syncScheduledJobFromConversation(
  conversationId: string,
  patch: { title?: string; prompt?: string; sourceWorkdir?: string | null }
): void {
  const conversation = conversationStore.get(conversationId)
  if (!conversation || conversation.sessionKind !== 'timer' || conversation.timerRunId) return
  const job =
    (conversation.timerJobId ? timerStore.getJob(conversation.timerJobId) : undefined) ??
    timerStore.getJobForConversation(conversationId)
  if (!job) return
...
const daemonAttach = new DaemonAttachService({
  ...
  fileSessions: createFileSessionCatalog({
    store: fileSessionStore,
    conversations: conversationStore,
    settings: settingsStore
  }),
  changeSets: createChangeSetCatalog(changeSetStore),
  accounts: createAccountsCatalog({
    accounts: accountStore,
    secrets: secretStore,
    settings: settingsStore,
    conversations: conversationStore
  }),
  settings: createSettingsCatalog(settingsStore, secretStore, {
    hasVavKey: () => vavAccountKeyPresent(accountStore, secretStore)
  }),
  timers: createTimerCatalog({
    store: timerStore,
  ...
  connectors: createConnectorCatalog({
...
function localShellMachineId(): string | null {
...
function controlPlaneOwns(conversation: Conversation | undefined | null): boolean {
  const machineId = remoteMachineId(conversation)
  if (machineId !== null) return daemonAttach.controlPlaneOf(machineId)
...
async function ensureLocalShellSession(conversation: Conversation): Promise<string | null> {
  ...
  if (conversation.workingDirectory) {
    try {
      await dial.setWorkspace(created, conversation.workingDirectory)
    } catch {
...
async function dialControl(
  ...
  if (isLocalMachine(conversation.machineId)) {
    let hostId: string | null = null
    try {
      hostId = await ensureLocalShellSession(conversation)
    } catch {
      return false
    }
    const shellId = localShellMachineId()
    const dial = shellId ? daemonAttach.controlOf(shellId) : undefined
    if (!hostId || !dial?.ready) return false
    await run(dial, hostId)
    return true
  }
  return forwardControl(conversation, run)
}
...
async function pullIfForwarded(
  ...
  if (!(await dialControl(conversation, run))) return false
  const machineId = remoteMachineId(conversation)
...
function applyDesktopControlEvent(machineId: string, message: RemoteServerMessage): void {
  ...
  if (message.type === 'turn') {
    ...
    if (
      ...
      refreshHostSession(machineId, message.conversationId)
    }
    return
  }
}
...
function refreshHostSession(machineId: string, hostConversationId: string): void {
  ...
  const adoptAs = hostRegistry.get(machineId)?.info.localShell ? LOCAL_MACHINE_ID : machineId
  void client
    .request('sessions.get', { id: hostConversationId }, 15_000)
    .then((got) => {
...
function controlPlaneTurnStatus(id: string): TurnStatus | null {
  ...
  const machineId = remoteMachineId(conversation) ?? localShellMachineId()
...
function attachLocalShellLogs(machineId: string): void {
  const host = hostRegistry.get(machineId)?.info
...
async function pullRemoteWorkspaceNow(id: string): Promise<void> {
  ...
  const adoptAs = hostRegistry.get(id)?.info.localShell ? LOCAL_MACHINE_ID : id
  let sessionsChanged = false
  for (const raw of catalog.sessions) {
    const adopted = conversationStore.adoptHostConversation(raw as Conversation, adoptAs)
    if (adopted) sessionsChanged = true
    const conversation =
      conversationStore.get((raw as Conversation).id) ??
      conversationStore.findOnHost(adoptAs, (raw as Conversation).id)
...
function syncHostWindows(hosts: WorkspaceHostInfo[]): void {
  if (mainWindow && !mainWindow.isDestroyed()) {
    ...
    if (current) mainWindow.setTitle(hostWindowTitle(current.id, current.name))
  }
...
function activateMainShellMachine(machineId: string): void {
...
async function getInfoOnMachine(
  machineId: string,
  path: string
): Promise<{ ok: true } | { ok: false; error: string }> {
  if (!path) return { ok: false, error: 'empty path' }
  if (isLocalMachine(machineId)) return fileService.getInfo(path)
  const host = hostRegistry.hostFor(machineId)
  if (!host.info.online) return { ok: false, error: `${host.info.name} is offline` }
  const cmd = getInfoSpawn(host.info.platform, path)
  if (!cmd) return { ok: false, error: 'Get Info is only available on macOS and Windows' }
  spawnOnHost(machineId, cmd.file, cmd.args)
  return { ok: true }
}

async function copyAsFileOnMachine(
  machineId: string,
  paths: string[]
): Promise<{ ok: true } | { ok: false; error: string }> {
  const list = (Array.isArray(paths) ? paths : []).filter((path) => Boolean(path))
  if (list.length === 0) return { ok: false, error: 'no paths' }
  if (isLocalMachine(machineId)) return fileService.copyAsFile(list)
  const host = hostRegistry.hostFor(machineId)
  if (!host.info.online) return { ok: false, error: `${host.info.name} is offline` }
  const cmd = copyAsFileSpawn(host.info.platform, list)
  if (!cmd) return { ok: false, error: 'Copy file is only available on macOS and Windows' }
  spawnOnHost(machineId, cmd.file, cmd.args)
  return { ok: true }
}
...
return c as text`
  ...
  registerLogIpc(ipcMain, logStore, {
    ...
    remote: () => {
      const client = daemonAttach.localShellClient()
      return client ? createDaemonLogRemote(client) : null
    }
  })
  ...
  registerConversationMetaIpc(ipcMain, conversationStore, {
    ...
    onRenamed: (id, title) => {
      syncScheduledJobFromConversation(id, { title })
    },
    onArchive: (id) => {
    ...
    forwardPin: async (id, pinned) => {
      const conversation = conversationStore.get(id)
      await dialControl(conversation, (dial, hostId) => dial.pin(hostId, pinned))
      return false
    },
    ...
    forwardSetLeaf: async (id, leafId) => {
      const conversation = conversationStore.get(id)
    ...
    forwardDuplicate: async (id) => {
      ...
      const used = await dialControl(conversation, async (dial, hostId) => {
        createdId = await dial.duplicateSession(hostId)
      })
      if (!used || !createdId) return null
      const machineId = remoteMachineId(conversation) ?? localShellMachineId()
      if (machineId) await pullRemoteWorkspace(machineId)
      const adopted = conversationStore.get(createdId)
      return adopted ? conversationToMeta(adopted) : null
    },
    forwardContinue: async (id, messageId) => {
      const conversation = conversationStore.get(id)
      if (!controlPlaneOwns(conversation)) return undefined
      let createdId: string | null = null
      const used = await dialControl(conversation, async (dial, hostId) => {
        createdId = await dial.continueSession(hostId, messageId)
      })
      if (!used || !createdId) return null
      const machineId = remoteMachineId(conversation) ?? localShellMachineId()
      if (machineId) await pullRemoteWorkspace(machineId)
  ...
  registerConversationMutateIpc(ipcMain, conversationStore, {
    ...
    forwardRemove: async (ids) => {
      for (const id of ids) {
        const conversation = conversationStore.get(id)
        await dialControl(conversation, (dial, hostId) => dial.archive(hostId))
      }
    },
    createOnRemote: async (options) => {
      const machineId = options?.machineId ?? LOCAL_MACHINE_ID
      if (isLocalMachine(machineId)) {
        await waitForMountedLocalShell(
          hostRegistry,
          (id) => daemonAttach.waitForControlPlane(id),
          localShellPairing
        )
      }
      const shellId = localShellMachineId()
      const control = isLocalMachine(machineId)
        ? shellId
          ? daemonAttach.controlOf(shellId)
          : undefined
        : daemonAttach.controlOf(machineId)
      ...
      const inheritedModel = options?.model?.trim()
      if (inheritedModel) control.configure(id, { model: inheritedModel })
      const pullId = isLocalMachine(machineId) ? shellId! : machineId
      await pullRemoteWorkspace(pullId)
      const adopted =
        conversationStore.get(id) ??
        conversationStore.findOnHost(isLocalMachine(machineId) ? LOCAL_MACHINE_ID : machineId, id)
      if (!adopted) return null
      const inherited: { swarmParentId?: string | null; model?: string } = {}
      if (options?.swarmParentId) inherited.swarmParentId = options.swarmParentId
      if (inheritedModel) inherited.model = inheritedModel
      if (Object.keys(inherited).length) {
        conversationStore.updateMeta(adopted.id, inherited)
      }
      lastSeenConversationId = adopted.id
      publishConversations()
      return conversationToMeta(conversationStore.get(adopted.id) ?? adopted)
    },
    forwardConfigure: async (id, patch) => {
      const conversation = conversationStore.get(id)
    ...
    forwardSelectBranch: async (id, messageId) => {
      const conversation = conversationStore.get(id)
      if (!controlPlaneOwns(conversation)) return undefined
      let leaf: string | null = null
      const used = await dialControl(conversation, async (dial, hostId) => {
        dial.setLeaf(hostId, messageId, true)
        const thread = await dial.waitThread(hostId)
        leaf = thread?.messages.at(-1)?.id ?? null
      })
      return used ? leaf : null
    },
  ...
  registerAgentIpc(
    ...
    conversationStore,
    {
      ...
      tryRemoteCancel: (id) => {
        const remote = conversationStore.get(id)
        if (!remote) return false
        void dialControl(remote, (dial, hostId) => dial.cancel(hostId)).then((used) => {
          if (used) return
          if (agentFor(id) === 'cli') cliHost.cancel(id)
          else agent.cancel(id)
        })
        return Boolean(localShellMachineId() || remoteMachineId(remote))
      },
      tryRemoteAnswer: async (id, toolCallId, answer) => {
        const remote = conversationStore.get(id)
        if (!remote) return false
        return dialControl(remote, (dial, hostId) => dial.reply(hostId, toolCallId, answer))
      },
      tryRemoteRegenerate: async (id, messageId) => {
        const conversation = conversationStore.get(id)
        if (!controlPlaneOwns(conversation)) return false
        return dialControl(conversation, (dial, hostId) => dial.regenerate(hostId, messageId))
      },
      tryRemoteEdit: async (id, messageId, text) => {
        const conversation = conversationStore.get(id)
        if (!controlPlaneOwns(conversation)) return false
        return dialControl(conversation, (dial, hostId) => dial.edit(hostId, messageId, text))
      },
      tryRemoteFork: async (id, messageId) => {
        const conversation = conversationStore.get(id)
        if (!controlPlaneOwns(conversation)) return undefined
        let leaf: string | null = null
        const used = await dialControl(conversation, async (dial, hostId) => {
          dial.fork(hostId, messageId)
          const thread = await dial.waitThread(hostId)
          leaf = thread?.messages.at(-1)?.id ?? '@root'
        })
        return used ? leaf : null
      },
      tryRemoteCompact: async (id, options) => {
        const conversation = conversationStore.get(id)
        if (!controlPlaneOwns(conversation)) return null
        let result: { ok: boolean; error?: string; compaction?: unknown } | null = null
        const used = await dialControl(conversation, async (dial, hostId) => {
          result = await dial.compact(hostId, options?.keepAfterMessageId)
        })
        return used ? result : null
      },
      tryRemoteClearCompaction: async (id, leafId) => {
        const conversation = conversationStore.get(id)
        if (!controlPlaneOwns(conversation)) return null
        let result: { ok: boolean; error?: string } | null = null
        const used = await dialControl(conversation, async (dial, hostId) => {
          result = await dial.clearCompaction(hostId, leafId)
        })
        return used ? result : null
      },
  ...
  registerFilesIpc(ipcMain, fileService, workingCopyService, {
    ...
    getInfoOn: getInfoOnMachine,
    copyAsFileOn: copyAsFileOnMachine,
  ...
  registerPluginIpc(ipcMain, pluginService, fileService, () => daemonAttach.localShellClient() ?? null)
  ...
  registerAgentsIpc(ipcMain, {
    ...
    probeLocal: async (specs, force) => {
      ...
      if (shellId) {
        if (force) await daemonAttach.probeProviders(shellId)
        const found = daemonAttach.providersOf(shellId)
        const byId = new Map(found.map((row) => [row.id, row.path]))
        const out: Record<string, string | null> = {}
        for (const spec of specs) {
          out[spec.id] = byId.get(spec.id) ?? daemonAttach.whichCached(shellId, spec.candidates)
        }
        return out
      }
      return probeAgentExecutables(specs, { force })
    },
  ...
  registerChangeSetIpc(ipcMain, changeSetStore, () => daemonAttach.localShellClient() ?? null)
...
function projectSmokeChangeReview(
  conversationId: string,
  set: { id: string; files?: unknown[] },
  user?: ChatMessage | null,
  assistant?: ChatMessage | null
): void {
  const conversation = conversationStore.get(conversationId)
  if (!conversation?.messages.some((row) => row.changeSetId === set.id)) {
    if (user) {
  ...
  const messageId =
    assistant?.id ??
    conversationStore.get(conversationId)?.messages.find((row) => row.changeSetId === set.id)?.id
  handleAgentEvent({
    type: 'change-review',
    conversationId,
    changeSetId: set.id,
    pendingCount: Array.isArray(set.files) ? set.files.length : 2,
    changeSet: set as never,
    messageId
  })
  publishConversations()
}

function wantsSmokeChangeReview(): boolean {
  return process.env.VAV_SMOKE_SEED === '1' || process.env.VAV_E2E_SEED_REVIEW === '1'
}

async function waitForMainRenderer(): Promise<void> {
  const wc = mainWindow?.webContents
  if (!wc || wc.isDestroyed() || !wc.isLoading()) return
  await new Promise<void>((resolve) => wc.once('did-finish-load', () => resolve()))
}

let smokeReviewStarted = false

async function maybeSeedSmokeChangeReview(): Promise<void> {
  if (!wantsSmokeChangeReview() || smokeReviewStarted) return
  smokeReviewStarted = true
  await waitForMainRenderer()
  await seedSmokeChangeReview()
}

/** Dev/smoke: freeze a 2-file ChangeSet on the local service, then paint it. */
async function seedSmokeChangeReview(): Promise<void> {
  ...
  for (let i = 0; i < 40 && !meta; i++) {
    ...
    meta = conversationStore.listMeta().find((c) => !c.archived)
  }
  if (!meta) {
  ...
  const seeded = await seedChangeReviewTurn({
    ...
    workdir,
    model: meta.model || 'test',
    changeSets: changeSetStore,
    appendMessages: (user, assistant) => {
      conversationStore.appendMessage(meta.id, user)
      conversationStore.appendMessage(meta.id, assistant)
      conversationStore.flush()
      handleAgentEvent({ type: 'user', conversationId: meta.id, message: user })
      handleAgentEvent({
        type: 'end',
        conversationId: meta.id,
        message: assistant,
  ...
  const set = changeSetStore.get(seeded.setId)
  handleAgentEvent({
    type: 'change-review',
    conversationId: meta.id,
    changeSetId: seeded.setId,
    pendingCount: seeded.pendingCount,
    changeSet: set ?? undefined,
    messageId: conversationStore.get(meta.id)?.messages.find((row) => row.changeSetId === seeded.setId)
      ?.id
  })
  console.log('[smoke] seeded change review', seeded.setId, 'conv', meta.id)
}
...
if (singleInstance) {
  ...
  app.whenReady().then(async () => {
    ...
    if (shouldRestoreInProcessPty(process.env, process.argv, { packaged: app.isPackaged })) {
    ...
    if (vavServerPairing) {
      ...
      void pairing.then((result) => {
        ...
        if (!result.host.localShell) {
          ...
          void maybeSeedSmokeChangeReview()
        }
      })
    }
    ...
    if (mainWindow.webContents.isLoading()) {
      mainWindow.webContents.once('did-finish-load', onMainLoaded)
    } else {
      onMainLoaded()
    }
