...
final class RemoteClient: NSObject, ObservableObject, UNUserNotificationCenterDelegate {
    ...
    @Published private(set) var recoveries: [String: RemoteTurnRecovery] = [:]
    @Published private(set) var reviews: [String: RemoteReviewSet] = [:]
    ...
    private var requestedReviews = Set<String>()
    ...
    func compact(conversationId: String) {
        guard let line = ClientFrame.compact(conversationId: conversationId) else { return }
        write(line)
    }

    func regenerate(conversationId: String, messageId: String) {
        guard let line = ClientFrame.regenerate(conversationId: conversationId, messageId: messageId) else { return }
        write(line)
        setGenerating(conversationId, true)
    }

    func edit(conversationId: String, messageId: String, text: String) {
        guard let line = ClientFrame.edit(conversationId: conversationId, messageId: messageId, text: text) else { return }
        write(line)
        setGenerating(conversationId, true)
    }

    func fork(conversationId: String, messageId: String) {
        guard let line = ClientFrame.fork(conversationId: conversationId, messageId: messageId) else { return }
        write(line)
    }

    func duplicate(conversationId: String) {
        guard let line = ClientFrame.duplicate(conversationId: conversationId) else { return }
        write(line)
    }

    func continueInNew(conversationId: String, messageId: String) {
        guard let line = ClientFrame.continueSession(conversationId: conversationId, messageId: messageId) else { return }
        write(line)
    }

    func applyGoal(conversationId: String, action: String, objective: String? = nil) {
        guard let line = ClientFrame.goal(conversationId: conversationId, action: action, objective: objective) else { return }
        write(line)
    }

    func review(conversationId: String, action: String, setId: String? = nil) {
        guard let line = ClientFrame.review(conversationId: conversationId, action: action, setId: setId) else { return }
        write(line)
    }

    private func requestMissingReviews(conversationId: String, messages: [RemoteThreadMessage]) {
        for message in messages {
            guard let setId = message.changeSetId, !setId.isEmpty else { continue }
            if reviews[setId] != nil || requestedReviews.contains(setId) { continue }
            requestedReviews.insert(setId)
            review(conversationId: conversationId, action: "get", setId: setId)
        }
    }

    func locateWorkspace(conversationId: String, destinationDir: String) {
        guard let line = ClientFrame.locate(conversationId: conversationId, destinationDir: destinationDir) else { return }
        write(line)
    }

    func deleteMessage(conversationId: String, messageId: String) {
        guard let line = ClientFrame.deleteMessage(conversationId: conversationId, messageId: messageId) else { return }
        write(line)
    }

    func setLeaf(conversationId: String, messageId: String) {
        guard let line = ClientFrame.leaf(conversationId: conversationId, messageId: messageId) else { return }
        write(line)
    }
    ...
    private func handle(_ frame: ServerFrame) {
        ...
        case .thread(let conversationId, let messages):
            ...
            requestMissingReviews(conversationId: conversationId, messages: threads[conversationId] ?? [])
        case .reviewed(_, let ok, let set):
            if ok, let set {
                reviews[set.id] = set
    ...
    private func applyTurn(_ turn: RemoteTurn) {
        ...
        case "running":
            ...
            recoveries[turn.conversationId] = turn.recovery
        ...
        case "done", "error", "cancelled":
            ...
            recoveries[turn.conversationId] = nil
