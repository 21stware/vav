...
struct SessionDetailView: View {
    ...
    @State private var pickLocate = false
    ...
    @State private var editMessageId = ""
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    ...
                    if liveSession.temporary {
                        Button("定位到文件夹") { pickLocate = true }
                    }
                    Button("设置目标") {
                        goalTitle = liveSession.goal?.objective ?? ""
                        showGoal = true
                    }
                    Button("新建临时工作区") {
                    ...
                    Button("压缩上下文") {
                        client.compact(conversationId: session.id)
                    }
                    Button("复制会话") {
                        client.duplicate(conversationId: session.id)
                    }
                    if let last = messages.last {
                        Button("在新会话继续") {
                            client.continueInNew(conversationId: session.id, messageId: last.id)
                        }
                    }
                    if let assistant = messages.last(where: { $0.role == "assistant" }) {
                        Button("重新生成") {
                            client.regenerate(conversationId: session.id, messageId: assistant.id)
                        }
                    }
                    if let user = messages.last(where: { $0.role == "user" }) {
                        ...
                        Button("分叉") {
                            client.fork(conversationId: session.id, messageId: user.id)
                        }
                    }
                    if let last = messages.last {
                        Button("删除上一条", role: .destructive) {
                            client.deleteMessage(conversationId: session.id, messageId: last.id)
                        }
                        Button("设为当前叶子") {
                            client.setLeaf(conversationId: session.id, messageId: last.id)
                        }
                    }
                    if isRunning {
        ...
        .sheet(isPresented: $pickWorkspace) {
            WorkspacePickerView(conversationId: session.id, purpose: .workdir)
                .environmentObject(client)
        }
        .sheet(isPresented: $pickLocate) {
            WorkspacePickerView(conversationId: session.id, purpose: .locate)
                .environmentObject(client)
        }
        ...
        .alert("设置目标", isPresented: $showGoal) {
            TextField("目标", text: $goalTitle)
            Button("取消", role: .cancel) {}
            Button("保存") {
                let text = goalTitle.trimmingCharacters(in: .whitespacesAndNewlines)
                if !text.isEmpty { client.applyGoal(conversationId: session.id, action: "set", objective: text) }
            }
        }
        .alert("编辑上一条", isPresented: $showEdit) {
            TextField("消息", text: $editTitle)
            Button("取消", role: .cancel) {}
            Button("发送") {
                let text = editTitle.trimmingCharacters(in: .whitespacesAndNewlines)
                if !text.isEmpty, !editMessageId.isEmpty {
                    client.edit(conversationId: session.id, messageId: editMessageId, text: text)
                }
            }
        }
        .onAppear {
    ...
    private var transcript: some View {
        ScrollViewReader { proxy in
            ScrollView {
                    ...
                    if let goal = liveSession.goal {
                        GoalBannerView(goal: goal) { action in
                            client.applyGoal(conversationId: session.id, action: action)
                        }
                    }
                    ...
                    ForEach(messages) { message in
                        ThreadTurn(
                            message: message,
                            review: message.changeSetId.flatMap { client.reviews[$0] },
                            onReply: { toolCallId, answer in
                                client.reply(conversationId: session.id, toolCallId: toolCallId, answer: answer)
                            },
                            onReview: { setId, action in
                                client.review(conversationId: session.id, action: action, setId: setId)
                            }
                        )
                        .id(message.id)
                    }
                    ...
                    if showLiveTurn {
                        LiveAgentTurn(
                            ...
                            draft: liveDraft,
                            recovery: client.recoveries[session.id]
                        ) { toolCallId, answer in
                            client.reply(conversationId: session.id, toolCallId: toolCallId, answer: answer)
                        }
                        .id("generating")
                    }
...
private struct LiveAgentTurn: View {
    ...
    let thinking: String
    let draft: String
    var recovery: RemoteTurnRecovery?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let recovery {
                Text(turnRecoveryLabel(recovery.kind))
                    .font(.system(size: 12, weight: .medium))
                    .foregroundStyle(.secondary)
                    .accessibilityIdentifier("stream-status")
            }
            if !blocks.isEmpty {
...
private struct ThreadTurn: View {
    ...
    var review: RemoteReviewSet?
    var onReply: (String, String) -> Void = { _, _ in }
    var onReview: (String, String) -> Void = { _, _ in }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let setId = message.changeSetId, !setId.isEmpty {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if let reviewStatus {
                        Text(reviewStatus == "accepted" ? "已接受" : "已拒绝")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    } else {
                        HStack(spacing: 12) {
...
private struct GoalBannerView: View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 12) {
                ...
                if goal.status == "paused" || goal.status == "blocked" || goal.status == "limited" {
                    Button("继续") { onAction("resume") }
                }
                Button("清除") { onAction("clear") }
            }
            .font(.caption)
        }
        .padding(10)
