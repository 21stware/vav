...
function sendDaemon(obj) {
  if (daemonWs && daemonWs.readyState === 1) daemonWs.send(JSON.stringify(obj))
}

function attachDaemon(found) {
  if (daemonWs) {
    daemonWs.onclose = null
    daemonWs.close()
  }
  daemonWelcome = null
  daemonWs = new WebSocket(found.wsUrl)
  daemonWs.onopen = () => {
    sendDaemon({ type: 'hello', proto: 1, auth: found.secret, role: 'daemon', device: DEVICE })
  }
  daemonWs.onmessage = (event) => {
    for (const line of String(event.data).split('\n').filter(Boolean)) {
      try {
        const payload = JSON.parse(line)
        if (payload.type === 'welcome') daemonWelcome = payload
        broadcast({ type: 'daemon', payload })
      } catch {
        /* ignore a bad line */
      }
    }
...
function persistHint(found) {
  const origin = loopbackWebOrigin(found.origin)
  const url = new URL(origin)
  void chrome.storage.local.set({
    vavDiscoverHint: {
      origin,
      wsUrl: loopbackWsUrl(found.wsUrl || wsUrlFromOrigin(origin)),
      ports: [Number(url.port) || 4752],
      hosts: [pairingHost(url.hostname)],
...
function attachSocket(found) {
  ...
  ws.onopen = () => {
    ...
    helloTimer = setTimeout(() => {
      setStatus('reconnecting', { error: 'Pairing timed out — retrying' })
      ws?.close()
    }, 8_000)
  }
  ws.onerror = () => {
  ...
  ws.onclose = () => {
    ...
    ws = null
    if (state.status === 'connected' || state.status === 'searching' || state.status === 'reconnecting') {
      setStatus('reconnecting', { error: 'Disconnected — retrying' })
      scheduleReconnect(1_200)
    } else if (state.status === 'error') {
      scheduleReconnect(2_400)
    }
  }
  ws.onmessage = (event) => {
    for (const line of String(event.data).split('\n').filter(Boolean)) {
...
function hintTarget(saved) {
  if (saved.origin && saved.secret) {
    const origin = loopbackWebOrigin(saved.origin)
    return {
      origin,
      wsUrl: loopbackWsUrl(saved.wsUrl || wsUrlFromOrigin(origin)),
      secret: saved.secret,
      name: 'VAV'
    }
  }
  if (saved.wsUrl && saved.secret) {
    const origin = loopbackWebOrigin(
      saved.origin || saved.wsUrl.replace(/^ws/, 'http').replace(/\/vav$/, '')
    )
    return { origin, wsUrl: loopbackWsUrl(saved.wsUrl), secret: saved.secret, name: 'VAV' }
  }
  const host = saved.hosts?.[0]
  if (saved.secret && host) {
    const clean = pairingHost(host)
    const authority = clean.includes(':') && !clean.startsWith('[') ? `[${clean}]` : clean
    const origin = `http://${authority}:4752`
...
async function ensureLanPermission(host) {
  if (!host || isLoopbackAddress(host) || !isPrivateLanAddress(host)) return true
...
async function pairManual(text) {
  ...
  if (!(await ensureLanPermission(parsed.host))) {
    setStatus('error', { error: 'Chrome needs permission to reach that LAN host.' })
    return
  }
...
function replayPhone(port) {
  port.postMessage({ type: 'state', state: snapshot() })
  if (state.status === 'connected') {
    port.postMessage({
      type: 'wire',
      payload: { type: 'welcome', proto: 1, version: state.version, host: state.host }
    })
  }
  if (state.host) port.postMessage({ type: 'wire', payload: state.host })
  port.postMessage({ type: 'wire', payload: { type: 'sessions', sessions: state.sessions } })
  for (const row of Object.values(state.controls)) {
    if (row) port.postMessage({ type: 'wire', payload: row })
  }
  if (state.active && state.threads[state.active]) {
    port.postMessage({
      type: 'wire',
      payload: { type: 'thread', conversationId: state.active, messages: state.threads[state.active] }
    })
  }
  if (daemonWelcome) port.postMessage({ type: 'daemon', payload: daemonWelcome })
}
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  port.onMessage.addListener(async (msg) => {
    ...
    if (msg.type === 'daemon-wire' && msg.payload) {
      sendDaemon(msg.payload)
      return
    }
