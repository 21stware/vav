 ...
 * Desktop Connect copies `vavrtp://…` (daemon TCP; legacy `vav-daemon://`).
 * The extension talks HTTP/WS on the web bridge (default 4752), which shares
 * that secret.
 */
export function parsePairing(text) {
  ...
  if (/^https?:\/\//i.test(trimmed) || /^wss?:\/\//i.test(trimmed)) {
    try {
      ...
      const host = pairingHost(advertised)
      const localOrigin = loopbackWebOrigin(origin)
      return {
        host,
        origin: localOrigin,
        wsUrl: loopbackWsUrl(wsUrlFromOrigin(localOrigin, wsPath)),
        ...(secret ? { secret } : {})
      }
    } catch {
      return null
