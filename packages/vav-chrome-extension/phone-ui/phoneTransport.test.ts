...
describe('extension MV3 port', () => {
  ...
  it('replays host / sessions from the service-worker snapshot', async () => {
    const ports: ReturnType<typeof mockPort>[] = []
    const transport = createExtensionTransport(() => {
      const port = mockPort()
      ports.push(port)
    ...
    const lines: string[] = []
    transport.onLine((line) => lines.push(String(line.type || '')))
    ports[0]?.emit({
      type: 'state',
      state: {
        status: 'connected',
        ...
        controls: {
          s1: { type: 'controls', conversationId: 's1', model: 'deepseek-chat', models: [{ id: 'deepseek-chat', label: 'DeepSeek' }] }
        }
      }
    })
    assert.ok(lines.includes('welcome'))
    assert.ok(lines.includes('host'))
    assert.ok(lines.includes('sessions'))
    assert.ok(lines.includes('controls'))
    assert.equal(transport.pageState().title, '')
  })
