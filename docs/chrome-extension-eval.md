...
评估对象：把 VAV 的自定义模型、以及「跟本机 `vav-server` 通信并使用 agent」做成 Chrome 扩展，值不值得做、做成什么样、贵在哪里。
...
| 扩展能不能直接跟本机 `vav-server` 说话？ | 不能。`vav-server` 是裸 TCP JSON-lines，浏览器没有任意 TCP。必须加一层 WebSocket / HTTP 网关，或走 Native Messaging。 |
| 扩展能不能「用上 agent」？ | 能，但 **循环不能跑在扩展进程里**。`vav-server` 的定义就是 headless VAV（daemon）；README 也写了 host agent turns。落地却只交了 workspace-host 那一半，所以今天桌面配对后只好自己跑循环。扩展要么先挂桌面（A），要么把 daemon 缺的那一半补齐再直连（C）。 |
...
成本按「要动哪些子系统、侵入多深」计，不按日历估。下文四条路径里，**路径 A 是推荐起步**；路径 C 是把 `vav-server` 已承诺的另一半补齐，扩展才能不经桌面当它的壳。

B 和 C 不是深浅：B 是扩展自己去调模型 API（会说话，不会干活）；C 是扩展当 `vav-server` 的遥控器（会改文件、跑 shell）。C 贵在补齐 daemon 里还没落地的 agent / 会话 / 密钥，不是改它是什么。见 §5。
...
桌面 / vav-server         ── hello.role=daemon ──► DaemonServer       fs / spawn / pty
```

关键事实：

1. **`vav-server` 的名字和 README 已经是 headless VAV。** `@21stware/vav-server` 写的是 host files、PTY、**and agent turns**，不打开桌面。实现只交了前两样：`DaemonServer` 的 fs / spawn / pty。phone-role 被拒、没有 `AgentRuntime`、`~/.vav-server` 不持密钥——那是落地缺口，不是「它其实是另一样东西」。
2. **所以今天桌面配对后，循环只好跑在桌面上。** `desktopClientAgainst('headless')` 把 `localAgent` 设回控制端，是对缺口的回退，不是 daemon 的终态。工具（`terminal` / `fs_*`）经 daemon RPC 打到 `vav-server`；DuckDB / `web_search` / 检索仍留在控制端。
...
### 2.1 浏览器连不上今天的 `vav-server`

传输是 **裸 TCP 上的换行 JSON**，不是 WebSocket，不是 HTTP。无 TLS。默认口：

| 通道 | 端口 | 谁在听 |
| --- | --- | --- |
| daemon | 4750 | `vav-server` / 桌面「允许其他设备」 |
...
| 桥 | 优点 | 代价 |
| --- | --- | --- |
| **给 `vav-server` / 桌面加 WebSocket（或 HTTP+SSE）网关** | 扩展用标准 `WebSocket`；大帧不受 Native Messaging 上限；以后 Web / 扩展 / 测试共用 | 要改宿主；Chrome 147+ 的 Local Network Access 要处理 |
| **Native Messaging host** | Chrome 官方推荐的本机通道；不碰 LNA | 每 OS 一份宿主安装与清单；**host → 扩展单条消息上限 1 MB**（`vav-server` 单帧上限是 8 MB）；等于再发一份本机二进制 |
...
- 网关绑 `127.0.0.1`，不要绑 `0.0.0.0`（`vav-server` 默认绑全接口是给 LAN 配对用的；给扩展的口必须 loopback）
...
| 桌面 VAV / 补齐后的 `vav-server` / Native Host | 行。长循环留在 Node |
...
「自定义模型」正好落在第一项上。「跟 `vav-server` 用 agent」落在它没有的那几项上，所以必须桥。
...
| --- | --- | --- | --- |
| **A. 薄客户端** | 桌面 / 未来的 vav-server 宿主 | 宿主 Keychain / `~/.vav-server` | 与 iOS 完全同构。扩展只画选择器 |
| **B. 扩展自己调供应商** | Service worker + `vavProtocol.ts` | `chrome.storage.local`（可选 session） | 能跑通 Chat；密钥安全降一档；和「local-first、密钥不上渲染层」冲突 |
| **C. 扩展调、工具走 vav-server** | SW 里跑循环 | 扩展存储 | 拓扑上像「桌面 → vav-server」，但循环在 SW 里，§2.3 否决 |
...
## 4. 跟本机 `vav-server` 通信并用 agent

### 4.1 今天桌面是怎么用 `vav-server` 的
...
`vav-server` 还提供 loopback 管理口（`vav-server clients | unpair | disconnect | rotate-offer`），与扩展无关。
...
| **补齐 `vav-server` 缺的那一半** | 把 control plane、`AgentRuntime`、密钥、会话放进已经在跑的 daemon 进程；`remoteHostKind` 里 `headless-daemon` 的 `localAgent` / `holdsSecrets` 改成和定义一致 | 「`npx vav-server` + 扩展」才能成立。这是把 README 已写的 agent turns 落地，不是改名 |
| **Native Messaging 宿主（迷你 main）** | 再发一份 Node 二进制：DaemonClient + AgentRuntime + SecretStore | `vav-server` 不用改循环，但安装面 ≈ 再装一个 VAV；1 MB 消息上限要处理 |
| **扩展 SW** | 见 §2.3 | 否决 |
...
| 工具 | 桌面 → vav-server | 扩展薄客户端（经宿主） | 扩展自己跑 |
| --- | --- | --- | --- |
...
| CLI agents（Claude Code / Codex / …） | 控制端 `proc.which` + 本地或远程 spawn | 只有宿主是桌面 / 补齐后的 vav-server 才有 | 不可能 |
...
- 对着 **`vav-server`**：要么暂时当 daemon 客户端（循环仍在桌面，半边货的回退），要么让 `vav-server` 接 `phone`（循环按定义就该在 daemon 里）。没有第三种语义。

---

## 5. 四条路径：潜力 × 成本

成本用「新子系统 + 要从 Electron main 抽出多少」衡量。潜力用「是否扩大 VAV 的工作台定位，而不是变成聊天包装」。
...
Side panel ──WS 127.0.0.1──► 桌面 RemoteControlHub (phone)
                                  ...
                                  └─ 若已配对 vav-server：工具仍打到 vav-server
```

- **用户得到什么：** 浏览时开着 VAV 会话；换模型（桌面已配好的 Custom 也在内）；发选区 / 当前页；回审批和 ask。若桌面已经挂了 `vav-server`，扩展间接用上那台机器的 agent 工具。
...
- **侵入：** 低。不补 `vav-server` 缺的那一半，也不改今天桌面回退跑循环的接法。
...
C:  扩展 ──WS 127.0.0.1──► vav-server（补齐 agent / 会话 / 密钥）──► 本机 fs / shell
                     └──────────────► 同一套自定义模型 API
    这是 daemon 的本义：无头 VAV，扩展只是又一个壳。
```

| | 路径 B | 路径 C |
| --- | --- | --- |
| 用户在 side panel 里做什么 | 和一个 LLM 聊天，可把当前页贴进去 | 对一台机器下指令，看着它改文件、跑命令 |
| 自定义模型谁来调 | **扩展自己** `fetch` 供应商 | **vav-server** 调供应商（扩展只发 `send` / `configure`） |
| API key 在哪 | `chrome.storage` | `~/.vav-server`（今天这里没有密钥这回事） |
| `vav-server` 要不要开 | 不要，连不上也没关系 | **必须开，并且缺的那一半（循环 / 密钥 / 会话）要先落地** |
| 会话、回合、工具卡片 | 扩展自己编一套，和桌面会话无关 | 和桌面 / iOS 同一套 phone 协议 |
| `terminal` / `fs_*` / 审批 / CLI agent | 没有 | 有（循环在 vav-server 里跑，和定义一致） |
| 扩展要写多少 | 设置页 + 流式聊天 | 和路径 A 几乎同一块 UI（phone 壳） |
| 贵的那一块 | 没有。协议层可搬 | **不在扩展，在把 daemon 已承诺的 agent 半边从 Electron main 抽进 `vav-server` 进程** |

B 的成本低，是因为它**不做 agent**。C 的成本高，也不是因为扩展难写，而是因为今天的 `vav-server` 故意不是 agent。

### 路径 B — 扩展里只做自定义模型 Chat（不连 vav-server）
...
### 路径 C — 扩展直连本机 `vav-server`（daemon 按定义接 agent）

这是题目里「和 local vav-server 通信使用 agent」的本义，也是 `vav-server` 这个名字已经承诺的形状：**VAV daemon = 无头 VAV**。扩展是壳，daemon 是家。不是「把 vav-server 变成另一样东西」。

贵，是因为 **daemon 只交了一半货。**

| | 定义 / README | 今天的二进制 |
| --- | --- | --- |
| 是什么 | Headless VAV，不打开桌面 | `@21stware/vav-server`，依赖只有 `node-pty` |
| 文件 / PTY / spawn | 要 | **有**（`DaemonServer`） |
| agent turns | README 写了要 host | **没有** `AgentRuntime`；phone hello 回 `control plane not available` |
| 密钥 / 会话 / 模型目录 | 无头 VAV 必须持有 | `~/.vav-server` 只有 identity + pairing secret |
| 桌面配对它时谁跑循环 | 按定义应是 daemon | 控制端回退自跑（`desktopClientAgainst('headless')`） |

`TECH_DESIGN.md` §15 那张表（vav-server：`localAgent: 否`、`holdsSecrets: 否`）记的是**这个缺口的现状**，不是「vav-server 其实不该跑 agent」。桌面回退跑循环，是一半货时唯一能用的接法。

扩展要直连 `vav-server` 用上 agent，缺的那一半得先落地：

1. **接 control plane。** 没有 `RemoteControlHub`，扩展连 `send` / `turn` / `configure` 都没处可去。Hub 已是 Electron-free，嵌进 daemon 是搬家，不是新协议。
2. **循环和密钥进同一个进程。** `~/.vav-server` 长出 SecretStore、accounts、conversations、模型探活。工具、sticky shell、DuckDB、skills、CLI agent 从 Electron main 抽进 daemon。桌面 main 里光 agent 相关就上万行，外加 pi-ai——这是账单的大头。
3. **给浏览器一座桥。** loopback WebSocket。和路径 A 给桌面加的是同一种适配器，本身不贵。

第 2 项贵，因为 agent 半边今天长在 Electron main 里，还没抽成 daemon 能用的库。不是重新定义 `vav-server`。抽完之后还要：

- 让 `remoteHostKind` 的 `headless-daemon` 和定义对齐（`localAgent` / `holdsSecrets` = true），e2e 从「phone 被拒」改成「phone 由 vav-server 跑回合」
- npm 包带上 pi-ai 和会话栈——`npx` 安装变重，但这是 headless VAV 本来的体积，不是额外产品
- 桌面 → `vav-server` 的回退拓扑：补齐之后应收回（回合回 daemon），和「桌面 → 另一台桌面」一样由宿主持有会话

收益：扩展、未来的 Web UI、iOS 都可以当同一只 daemon 的壳。这本来就是 daemon 的用途。

**不要**用「Native Host 里再塞一个 AgentRuntime」代替补齐——那是路径 D，在 daemon 外面再造一个脑子。

- **潜力：** 高。这是「VAV = daemon + 多种壳」已经写在名字里的那条路。
- **成本：** 高。贵在从 Electron main **抽出** agent 半边，不是另起一个产品。扩展壳和 A 同构。
- **建议：** 若战略上 `vav-server` 就是无头 VAV，C 是正路，A 只是半边货时的便宜壳。仍不必和「先验证 side panel 有人用」绑在同一刀；但不要把 C 说成改定义。

### 路径 D — Native Messaging 迷你 main + 现状 `vav-server`

拓扑上复制「桌面 → vav-server」：宿主跑循环和密钥，`vav-server` 只做 fs/pty。

- **看起来省：** 不动 `vav-server` 缺的那一半。
- **其实不省：** 要打包、签名、按 OS 注册 Native Host；Chrome 扩展 ID 写进 `allowed_origins`；自动更新是两份工件；1 MB 消息上限。用户心智是「装扩展还要再装一个后台」。
- **潜力：** 中。对已经会 `npx @21stware/vav-server` 的人重复。
...
第一刀   路径 A：扩展 = phone 客户端 + 页面上下文
              ...
              agent：桌面跑；若桌面已挂 vav-server，工具自然打过去
              协议：loopback WS；附件或「当前页当消息」

第二刀   补齐 vav-server 已承诺的 agent / 会话 / 密钥 + 同一套 WS
              扩展几乎不用改（还是 phone）
              这是 daemon 的本义，不是新身份
...
第一刀里和「自定义模型 / vav-server / agent」直接对应的交付：

1. **自定义模型** — 零新协议。宿主 `controls.models` 已经包含 Custom 探活出来的 id。扩展画现有 picker。
2. **vav-server** — 第一刀不直连 `vav-server`。用户在桌面 Settings 里配对 `vav-server`；扩展连桌面。这是今天唯一不破坏所有权的接法。
...
| `src/shared/remoteHostKind.ts` | 第一刀不改；路径 C 让 `headless-daemon` 和定义对齐 |
...
| 位置 | 做什么 |
| --- | --- |
| `vav-server.ts` / `DaemonServer.ts` | 接 `onControlHello`（daemon 本应有的 control plane） |
| `~/.vav-server` | 密钥、accounts、conversations |
| `AgentRuntime` + tools | 链到 `createLocalWorkspaceHost()` |
| `packages/vav-server` | 依赖和安装故事重写 |
| e2e `remote-daemon.spec.ts` | 补「phone → vav-server」而不再断言拒绝 |

---

## 9. 一句话对照

| 能力 | 第一刀（A） | 扩展直连现状 vav-server | 补齐 daemon 后（C） |
| --- | --- | --- | --- |
| 用桌面已配的自定义模型 | 有 | 无（agent 半边还没进进程） | 有（密钥进 `~/.vav-server`） |
| 在扩展里新加 Custom endpoint | 不做（避免第二套密钥） | — | 可做设置页，仍存宿主 |
| 发消息、看流式、回 ask | 有 | 无（phone 被拒） | 有 |
| 工具改 `vav-server` 那台机器的盘 / shell | 有（经桌面） | 无循环则无工具 | 有（daemon 自己跑，和定义一致） |
| 不打开桌面 | 无 | 无 | 有 |
| 当前页 / 选区进回合 | 第一刀的真正增量 | — | 同样带上 |

**潜力：** 作为「工作台的浏览器壳」是中高；作为「又一个自定义模型聊天框」是低。  
**成本：** A 是半边货时的便宜壳；C 是把 daemon 已承诺的 agent 半边从 Electron main 抽进 `vav-server`。两刀可以分开，但 C 不是改身份。
