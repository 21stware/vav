/**
 * Local admin channel for a running vav-server (list / disconnect / unpair / rotate).
 * `vav-server clients` talks to 127.0.0.1 via `admin.json`. If nothing is listening,
...
export type VavServerAdminHandlers = {
...
export function startVavServerAdmin(stateDir: string, handlers: VavServerAdminHandlers): Promise<Server> {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        reject(new Error('vav-server admin: no address'))
        return
      }
...
export function stopVavServerAdmin(stateDir: string, server: Server | null): void {
  try {
...
function attachAdminClient(socket: Socket, handlers: VavServerAdminHandlers): void {
...
function handleAdmin(raw: Record<string, unknown>, handlers: VavServerAdminHandlers): Record<string, unknown> {
  switch (raw.type) {
    case 'clients':
...
export async function runVavServerAdminCommand(
  stateDir: string,
...
function askAdmin(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      socket.destroy()
      reject(new Error('vav-server admin timed out'))
    }, 2000)
    timer.unref?.()
    socket.on('error', (err) => {
      clearTimeout(timer)
...
function runOfflineAdmin(
  ...
  if (command === 'disconnect') {
    return 'vav-server is not running — nothing to disconnect.\n'
  }
  if (command === 'unpair') {
    if (!id) return 'usage: vav-server unpair <grant-id>\n'
...
export function handleStdinLine(
  line: string,
  handlers: VavServerAdminHandlers
): string {
...
export function adminHandlersFor(server: DaemonServer, rotateOffer: () => string): VavServerAdminHandlers {
  return {
