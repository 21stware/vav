...
export class SkillService {
  ...
  loadLocal(
    ...
    const header = [
      ...
      `- WORKDIR is the conversation working directory. Put file intermediates and file deliverables there (slides, Office, HTML, scripts). A Note is \`note_write\` / \`note_edit\`. An Analysis object is \`analysis_write\` / \`analysis_edit\`. A schedule is \`schedule_write\` / \`schedule_edit\`. A Storage catalog file is \`storage_write\` / \`storage_edit\`. Do not write those as loose files in WORKDIR.`,
      ...
      `  - file deliverables: \`${work}/…\` (e.g. \`${work}/presentation.pptx\`). A note, analysis object, or schedule still goes to the app, not WORKDIR.`,
