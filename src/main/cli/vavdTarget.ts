/**
 * Resolve a running vavd (or desktop web-bridge) for vavc / vavcli.
 *
...
export type VavdTarget = VavdTcpTarget | VavdWsTarget
...
/** Candidate state dirs: flag, env, ~/.vavd, packaged-app spawn dir. */
export function defaultStateDirs(env: NodeJS.ProcessEnv = process.env, home = homedir()): string[] {
  ...
  if (process.platform === 'darwin') {
    add(join(appData, 'Library', 'Application Support', 'vav', 'vavd'))
    add(join(appData, 'Library', 'Application Support', 'vav-dev', 'vavd'))
  } else if (process.platform === 'win32') {
    const roaming = env.APPDATA || join(appData, 'AppData', 'Roaming')
    add(join(roaming, 'vav', 'vavd'))
    add(join(roaming, 'vav-dev', 'vavd'))
  } else {
    add(join(appData, '.config', 'vav', 'vavd'))
    add(join(appData, '.config', 'vav-dev', 'vavd'))
  }
  return out
}
...
export async function discoverLoopbackVavd(
  env: NodeJS.ProcessEnv = process.env
): Promise<VavdTarget | null> {
  const hint = env.VAVD_WEB_PORT ? [Number(env.VAVD_WEB_PORT)] : []
...
export type ResolveVavdTargetInput = {
  argv?: string[]
...
export async function resolveVavdTarget(input: ResolveVavdTargetInput = {}): Promise<VavdTarget> {
  ...
  const uri = argValueOrEq(argv, '--uri') || env.VAVD_URI
  if (uri) {
  ...
  if (input.discover !== false) {
    const discovered = await discoverLoopbackVavd(env)
    if (discovered) return discovered
  }
  throw new Error(
    'no pairing secret: pass --uri / --secret, run vavd first (~/.vavd), or leave the VAV app running'
  )
}

export function formatTarget(target: VavdTarget): string {
