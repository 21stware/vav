...
describe('previewOverlay', () => {
  it('treats app / xstate / clip html as an app overlay', () => {
    assert.equal(looksLikeAppClip('/tmp/vav-tuips/x/app.html'), true)
    assert.equal(looksLikeAppClip('/tmp/vav-tuips/x/xstate.html'), true)
    assert.equal(looksLikeAppClip('/tmp/vav-tuips/x/page.html'), true)
    assert.equal(looksLikeAppClip('/Users/me/site/index.html'), false)
  })

  it('treats conversation visuals as overlay previews', () => {
    assert.equal(looksLikeVisualOverlay('/tmp/vav-tuips/a/photo.png'), true)
  ...
  it('does not treat a File Session image as an overlay', () => {
    ...
    assert.equal(shouldOpenAsOverlay('/tmp/vav-tuips/a/photo.png'), true)
    assert.equal(shouldOpenAsOverlay('/tmp/vav-tuips/a/app.html'), true)
  })
})
