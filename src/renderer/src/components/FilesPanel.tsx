...
export function FilesPanel({ visible }: { visible: boolean }): React.JSX.Element {
  ...
  useEffect(() => {
    let cancelled = false
    if (!vercelTrayOn || !root || !window.vav?.connectors?.detect) {
      setHasVercel(false)
      return
    }
    void window.vav.connectors
      .detect(root)
      .then((rows) => {
        if (!cancelled) setHasVercel(rows.some((row) => row.id === 'vercel' && row.present))
      })
      .catch(() => {
      ...
      <div className="files-tray-stack">
        ...
        {vercelTrayOn && hasVercel ? (
          <div className="files-tray-pane" data-hidden={trayView !== 'vercel'}>
            <VercelPanel visible={visible && trayView === 'vercel'} onChrome={onVercelChrome} />
          </div>
        ) : null}
      </div>
    </>
