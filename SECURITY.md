...
- **Daemon / remote:** a valid pairing secret is equivalent to local code execution on the host. Desktop listen defaults to `127.0.0.1`. Headless `vav-server` still binds all interfaces so other devices can connect (`--listen 127.0.0.1` for loopback-only). Treat the pairing line like a password.
