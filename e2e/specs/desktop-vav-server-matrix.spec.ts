...
/**
 * Desktop UI over the spawned local vav-server — the production path.
 * Covers every named product-matrix branch on the workbench itself
 * (list / send / workspace / files / git / plugins / terminal / configure).
 */
test('spawned vav-server desktop covers list, send, files, and terminal', async () => {
  test.setTimeout(90_000)
  const harness = await launchVav({ spawnVavServer: true, stubTurn: true, seedGit: true })
  try {
    const { page } = harness
    await expect
      .poll(async () => {
    ...
    await page.locator('[data-testid="composer-input"]').fill('hello from spawned desktop vav-server')
    await page.locator('[data-testid="composer-send"]').click()
    await expect(page.getByText('e2e stub reply')).toBeVisible({ timeout: 20_000 })
    await expect
      .poll(() => readVavServerConversation(harness.userData, sessionId)?.includes('e2e stub reply') ?? false)
      .toBe(true)
