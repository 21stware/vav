...
test('Key Bindings shows send-key and accelerator groups', async () => {
  ...
  try {
    ...
    await expect(settings.getByText('Swarm', { exact: true })).toBeVisible()
    await expect(settings.getByText('Focus pane left')).toBeVisible()
    await settings.locator('[data-testid="settings-keybind-search"]').fill('pane')
    await expect(settings.getByText('Focus pane left')).toBeVisible()
    await expect(settings.getByText('Split pane right')).toBeVisible()
    await expect(settings.getByText('New session')).toHaveCount(0)
    await settings.locator('[data-testid="settings-keybind-search"]').fill('zzzz-nope')
    await expect(settings.locator('[data-testid="settings-keybind-empty"]')).toBeVisible()
    await settings.locator('[data-testid="settings-keybind-search"]').fill('')
